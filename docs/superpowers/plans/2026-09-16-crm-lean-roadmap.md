...
| AI Assistant V1A | **ACTIVE**（当前实现切片 = PR A — AI Foundation） | 只读 Ask / Analyze | 0.4–0.7× |
...
**AI Assistant V1A = ACTIVE**（当前实现切片 = PR A — AI Foundation）；**AI Assistant V1B = PLANNED**。完成 Hardening 本身不自动 Promote AI；本次是实现已开始后才切 ACTIVE。
...
→ AI Assistant V1A             ACTIVE（当前实现切片 = PR A — AI Foundation）
