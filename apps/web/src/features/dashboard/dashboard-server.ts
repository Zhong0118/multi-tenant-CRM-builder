...
import { parseDashboardConfigurationView, type DashboardOverview } from "./dashboard-types";
...
export async function loadDashboardOverview(tenantCode: string): Promise<DashboardOverview> { const client = await createServerApiClient(); const result = await client.GET(OVERVIEW_PATH, { params: { path: { tenantCode }, query: {} } }); if (!result.data) throw toApiError(result.error, result.response.status); return result.data as unknown as DashboardOverview; }
