...
const OVERVIEW_PATH =
  "/api/v1/workspaces/{tenantCode}/dashboard/overview" as const;

export async function loadDashboardConfiguration(tenantCode: string) {
...
export async function loadDashboardOverview(
  ...
  const client = await createServerApiClient();
  const result = await client.GET(OVERVIEW_PATH, {
