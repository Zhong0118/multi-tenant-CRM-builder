...
最新状态以 `docs/audits/2026-09-09/roles-and-readiness.md` 为准：独立环境创建公司出现 P2011/500，开通两套 E2E 未通过；邀请撤销存在竞态风险。两项均未修复，不得以先前部分测试通过宣布开通闭环完成。
...
- P7 第一刀：记录活动时间线。成员可对可见且可更新的记录追加 `NOTE / CALL / MESSAGE / MEETING`；历史不可编辑。独立的个人跟进待办已实现；对象关系和转换尚未开始。

### 产品与 UX 缺口

- 跟进待办目前只支持为自己安排事项；未接团队分派、主动提醒或异常队列。
...
2026-09-09 基线 main 与 origin/main 同步；本轮未提交改动位于 codex/crm-polish-followups。不要 reset、rebase、强推或部署。
