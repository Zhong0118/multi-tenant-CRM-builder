...
当前功能基线：本地 `main`，最近一项完整功能提交为 `0188cf2`（工作台 KPI 条与演示首页数据）

状态：仅本地开发与验证，未推送、未部署。相对 `origin/main` 约领先 49 个提交。
...
- 记录列表可导出当前筛选结果为 UTF-8 BOM CSV（Excel 可直接打开），列跟随个人列设置，上限 5000 条。导入尚未实现。
...
重要限制：工作台已能承载真实数据，但没有待办、逾期、异常队列等业务入口，不能视为最终 CRM 首页。
...
- P7 第一刀：记录活动时间线。成员可对可见且可更新的记录追加 `NOTE / CALL / MESSAGE / MEETING`；历史不可编辑。待办入口、逾期提醒、对象关系和转换尚未开始。

### 产品与 UX 缺口

- 工作台没有待办、下次跟进、逾期或异常提醒入口。
...
- 真实短信供应商。
- 待办、下次跟进提醒、逾期队列和个人任务入口。记录活动时间线已落地，活动上的 `nextActionAt` 尚未做成工作台入口。
- 状态机、对象关系、转换动作，以及重复客户识别、合并和去重。
- Excel 导入预览、字段映射和错误行回执。当前结果导出已落地。
- 批量操作、个人视图（筛选/排序命名保存）。列设置已落地。
- 附件字段背后的真实上传、下载、权限和对象存储。
- 真正的后台任务中心（当前操作页只列成功的模板应用）。
- 租户侧审计、导入导出、独立统计页仍是占位。
- 平台管理员受控进入租户协助排错的代管流程。
- 模板升级同步到已初始化公司。
- 生产数据库备份恢复、健康检查、监控告警、日志脱敏与保留、迁移回滚、域名、HTTPS、Cookie 和跨域配置。
...
P0–P5 主干已经落地。P7 已完成活动时间线、当前结果导出和个人列设置。表管理
下一刀是批量修改。导入必须带映射 UI。供应商凭据齐备后再接真实短信。不要反复
跑全仓测试；每个 Bug 一个聚焦复现。
