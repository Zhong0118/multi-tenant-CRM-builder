...
- Redis：端口 6379
- 已应用迁移：`0010_named_tenant_dashboards`
...
- 无。P4 的 `searchFieldKeys` 已从草稿保存、发布快照接到对象设计器和模板编辑器。旧发布版仍按“标题 + 可见默认列表文本列”搜索；显式空数组表示只搜标题。
...
- 真实短信供应商。
- 记录活动时间线、备注、电话/拜访记录、待办、下次跟进时间和逾期提醒。
...
P0–P5 主干已经落地，包括对象设计器的 searchFieldKeys 配置。不要重做侧栏拖拽、
多工作台或类型化筛选。供应商凭据齐备后再接真实短信。不要反复跑全仓测试；
每个 Bug 一个聚焦复现。
