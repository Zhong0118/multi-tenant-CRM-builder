...
PR A 已通过 PR #11 合并进入 `main`（合并提交 `bc6cad2`）。本阶段只把已稳定的
`pnpm --filter @crm/api test:e2e:critical` 接入 CI，并 Promote 为第六个 required check
`Critical API E2E`。**不改 `apps/**` / `packages/**` 产品逻辑或测试行为。**
