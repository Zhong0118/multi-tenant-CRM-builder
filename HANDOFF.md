...
- 记录列表第三次点击排序原本会忽略“取消排序”事件；现在会恢复到业务表发布时设置的默认排序，并保留搜索和筛选上下文。
...
- 侧边栏只能收起/展开，不能拖拽调整宽度。
- 普通记录列表尚无移动端卡片模式，小屏体验仍依赖横向表格或桌面布局。
- 当前数据模型是一家公司一套工作台，尚不支持多工作台创建、默认工作台和切换。
- 邀请接受后的反馈和 DRAFT 公司尚不能进入的原因不够直观。
- 公司名称当前可重复，与用户预期可能不一致，需要产品规则和界面提示。
- 记录搜索当前只查 `records.title`（即对象标题字段的派生值），不是首个展示列，也不是全字段搜索。
- 记录筛选当前只覆盖负责人和 `SINGLE_SELECT` 字段；尚不支持成员字段、文本、数字、金额、日期、布尔和多选字段的类型化筛选。
- 普通记录列表目前只在 UI 开放业务编号和最近更新时间排序；业务字段类型化排序尚未实现。
...
用户当前优先级是：先走通公司生命周期，然后尽快完成侧栏拖拽、移动端卡片和多工作台
切换，再做通用搜索筛选排序和工作台视觉；供应商凭据齐备后接真实短信。不要反复跑全仓
测试；每个 Bug 一个聚焦复现，完成一组任务后
只做一次人工 smoke。
